...
export async function groupsRoutes(fastify: FastifyInstance) {
    ...
    fastify.get("/", async (request, reply) => {
        try {
            ...
            const { data: groups, total } = await databaseService.findGroups({
    ...
    fastify.put("/:groupId/settings", async (request, reply) => {
        try {
            ...
            if (antilinkStatus !== undefined) {
                settings.antilink = { ...settings.antilink, status: antilinkStatus };
            }
            ...
            const group = await databaseService.updateGroupSettings(groupId, settings);
            ...
            reply.status(500).send({ error: "Failed to update group settings" });
        }
    });
        ...
        try {
            ...
            if (client) {
                try {
                    ...
                    console.log("Could not leave group (might already be removed):", leaveError);
                }
            }
            ...
            return {
                success: true,
                message: "Group deleted successfully and bot has left the group",
            };
        } catch (error) {
